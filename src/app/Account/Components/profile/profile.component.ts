...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.localStorage.getItem('AccountType')
      ? (this.aType = this.localStorage.getItem('AccountType'))
      : (this.aType = '');
    this.BindCountries();
    ...
    if (suspend) {
      ...
      this.CurrentStatus =
        'Your profile has been screened and rejected due to violation of our general terms and conditions. Your profile won’t be visible anywhere and you will not be able to do any actions except update your profile and photos until you fix any possible problems and issues.';
    }
  }
  ngOnDestroy() {
    ...
    if (this.CustomFieldsSubscription)
      this.CustomFieldsSubscription.unsubscribe();
    if (this.UpdateCustomFieldsSubscription)
      this.UpdateCustomFieldsSubscription.unsubscribe();
    if (this.MessageSubscription) this.MessageSubscription.unsubscribe();
    if (this.NewMessageSubscription) this.NewMessageSubscription.unsubscribe();
    if (this.MessageThreadSubscription)
    ...
    if (this.CheckCreditsSubscription)
      this.CheckCreditsSubscription.unsubscribe();
    if (this.ProfileViewSubscription)
  ...
  SetShownText() {
    ...
    if (this.userProfile.per_Info !== null) {
      if (this.userProfile.per_Info.length <= 500) {
        ...
        this.ShownPersonalInfo =
          this.userProfile.per_Info.substring(0, 500) + ' ...';
    ...
    if (this.userProfile.edu_info !== null) {
      if (this.userProfile.edu_info.length <= 500) {
        ...
        this.ShownEduInfo =
          this.userProfile.edu_info.substring(0, 500) + ' ...';
    ...
    if (this.userProfile.profinfo !== null) {
      if (this.userProfile.profinfo.length <= 500) {
        ...
        this.ShownProfInfo =
          this.userProfile.profinfo.substring(0, 500) + ' ...';
    ...
    if (this.userProfile.interstinfo !== null) {
      if (this.userProfile.interstinfo.length <= 500) {
        ...
        this.ShownIntInfo =
          this.userProfile.interstinfo.substring(0, 500) + ' ...';
    ...
    switch (boxName) {
      case 'summary':
        if (this.viewMoreLessSummary) {
          ...
          this.ShownSummary =
            this.userProfile.summary.substring(0, 500) + ' ...';
          this.viewMoreLessSummary = true;
        }
        break;
      case 'personal':
  ...
  CheckFormDirty() {
    if (this.ProfileForm !== null && this.ProfileForm !== undefined) {
      let dirty = false;
      if (
        (this.ProfileForm.dirty && !this.isSaved) ||
  ...
  SetMetaTags() {
    ...
    this.meta.SetPageTitle(
      this.userProfile.sname + ', ' + this.userLocation + ' | ispace1'
    );
  ...
  async BindCountries() {
    this.UpgradeSubscription = (await this.CountryService.getAll()).subscribe(
      (result: any) => {
        if (result) {
  ...
  async getUserProfile(isFromSaveImage?: boolean, isFirstLoad?: boolean) {
    ...
    this.parameterSubscription = this.activeRoute.params.subscribe(
      async (params) => {
        let Uname = params['UserName'];
        this.currentUser.uName = Uname;
        if (this.currentUser.uName) {
          this.subscribtion = (
            await this.GetProfileByuName.GetProfileByUname(this.currentUser)
          ).subscribe(
            (result: any) => {
              if (result) {
                ...
                if (
                  ...
                  +this.profileService.getProfileId() !=
                    this.checkUserProfile.id
                ) {
                  this.IsProfileForbidden = true;
                  this.router.navigate(['/Profiles'], {
                    queryParams: { forbidden: 'fb' },
                  });
                  return;
                }
                //check if blocked profile
                this.CheckBlockedProfileObject.UserId =
                  this.profileService.getProfileId();
                this.CheckBlockedProfileObject.AbuseProfileId =
                  this.checkUserProfile.id.toString();
                this.postingSubscription = this.profileService
                  .CheckBlockedProfile(this.CheckBlockedProfileObject)
                ...
                if (this.userProfile.canvasImageUrl) {
                  this.userProfile.canvasImageUrl.indexOf('ispace1-stripe') !=
                  -1
                ...
                if (this.userProfile.passportImageUrl) {
                  this.userProfile.passportImageUrl.indexOf(
                    'defaultProfileImage'
                  ) != -1
                ...
                this.passportImageUrl = this.GetProfileByuName.bindImageUrl(
                  'profile',
                  this.userProfile.passportImageUrl
                );
                this.canvasImageUrl = this.GetProfileByuName.bindImageUrl(
                  'posting',
                  this.userProfile.canvasImageUrl
                );
                //Bind Age
                this.Age = this.GetProfileByuName.CalculateAge(
  ...
  SetUserLocation(profile?: appRelatedProfiles) {
    ...
    if (profile) {
      ...
      country = this.profileService.getCountryAppriviation(
        this.userProfile.country
      );
  ...
  async GetPostingsList() {
    ...
    this.postingSubscription = (
      await this.MyArticlesListService.create(data)
    ).subscribe((result: any) => {
      if (result) {
  ...
  BindRelatedProfiles() {
    ...
    this.UpdateProfileSubscription = this.profileService
      .GetRelatedProfiles(this.userProfile.id)
      .subscribe((result: any) => {
        if (result) {
          ...
          if (this.fullRelatedProfilesList.length > 0) {
            ...
            for (let i = 4; i < 8; i++) {
              this.bottomRelatedprofilesList.push(
                this.fullRelatedProfilesList[i]
              );
            }
          }
        }
        ...
        if (result) {
          ...
          if (result.length < this.DailyLimit) {
            ...
            this.ConfirmDialogService.confirm(
              ...
              .then((confirmed) => {
                if (confirmed) {
                  this.blockedProfileObject.UserId =
                    this.ProfilesFavMeService.getProfileId();
            ...
            this.ConfirmDialogService.confirm(
              ...
              .then((confirmed) => {
                if (confirmed) {
                  ...
                  this.isUsedFullReportCredit = false;
                  this.reportedProfileObject.UserId =
                    this.ProfilesFavMeService.getProfileId();
                  this.reportedProfileObject.AbuseProfileId =
                    this.userProfile.id.toString();
                  this.reportedProfileObject.ReportCause = '';
                  this.reportedProfileObject.ReportDetails = '';
                  this.UpdateProfileSubscription = this.profileService
                    .ReportProfile(this.reportedProfileObject)
                    .subscribe((result: any) => {
                      if (result) {
                        this.router.navigate(['/Profiles']);
                      }
  ...
  async AddProfileView() {
    if (!this.profileOwner) {
      ...
      console.log(
        'ShownProfile> ' + this.profileViewObject.ShowenProfileUserId
      );
      console.log('mainProfile> ' + this.profileViewObject.UserId);
      this.ProfileViewSubscription = (
        await this.ProfileViewsService.create(this.profileViewObject)
      ).subscribe((result: any) => {
  ...
  bindUsedChars() {
    if (this.userProfile) {
      this.userProfile.photoTagline
        ? (this.photoTagLineRemainingChars =
            this.userProfile.photoTagline.length)
        : (this.photoTagLineRemainingChars = 0);
      this.userProfile.summary
        ? (this.summaryRemainingChars = this.userProfile.summary.length)
        : (this.summaryRemainingChars = 0);
      this.userProfile.per_Info
        ? (this.morePersonalRemainingChars = this.userProfile.per_Info.length)
        : (this.morePersonalRemainingChars = 0);
      this.userProfile.edu_info
        ? (this.moreEducationRemainingChars = this.userProfile.edu_info.length)
        : (this.moreEducationRemainingChars = 0);
      this.userProfile.profinfo
        ? (this.moreProfessionRemainingChars = this.userProfile.profinfo.length)
        : (this.moreProfessionRemainingChars = 0);
      this.userProfile.interstinfo
        ? (this.moreInterestsRemainingChars =
            this.userProfile.interstinfo.length)
        : (this.moreInterestsRemainingChars = 0);
      this.userProfile.city
        ? (this.cityRemainingChars = this.userProfile.city.length)
        : (this.cityRemainingChars = 0);
      this.userProfile.specialization
        ? (this.specializationRemainingChars =
            this.userProfile.specialization.length)
        : (this.specializationRemainingChars = 0);
      this.userProfile.hschool
        ? (this.highSchoolRemainingChars = this.userProfile.hschool.length)
        : (this.highSchoolRemainingChars = 0);
      this.userProfile.postgrade
        ? (this.postGradRemainingChars = this.userProfile.postgrade.length)
        : (this.postGradRemainingChars = 0);
      this.userProfile.ucollege
        ? (this.undergradCollegeRemainingChars =
            this.userProfile.ucollege.length)
        : (this.undergradCollegeRemainingChars = 0);
      this.userProfile.gradschool
        ? (this.gradSchoolRemainingChars = this.userProfile.gradschool.length)
        : (this.gradSchoolRemainingChars = 0);
      this.userProfile.memeberof
        ? (this.memberOfRemainingChars = this.userProfile.memeberof.length)
        : (this.memberOfRemainingChars = 0);
      this.userProfile.skill
        ? (this.keySkillsRemainingChars = this.userProfile.skill.length)
        : (this.keySkillsRemainingChars = 0);
      this.userProfile.achieve
        ? (this.mainAcheavmentRemainingChars = this.userProfile.achieve.length)
        : (this.mainAcheavmentRemainingChars = 0);
      this.userProfile.certifications
        ? (this.coreCertificationRemainingChars =
            this.userProfile.certifications.length)
        : (this.coreCertificationRemainingChars = 0);
      this.userProfile.cemp
        ? (this.currentEmployerRemainingChars = this.userProfile.cemp.length)
        : (this.currentEmployerRemainingChars = 0);
      this.userProfile.pemp
        ? (this.pastEmployerRemainingChars = this.userProfile.pemp.length)
        : (this.pastEmployerRemainingChars = 0);
      this.userProfile.interest
        ? (this.interestsRemainingChars = this.userProfile.interest.length)
        : (this.interestsRemainingChars = 0);
      this.userProfile.hobbies
        ? (this.hobbiesRemainingChars = this.userProfile.hobbies.length)
        : (this.hobbiesRemainingChars = 0);
      this.userProfile.favoriteMusic
        ? (this.favMusicRemainingChars = this.userProfile.favoriteMusic.length)
        : (this.favMusicRemainingChars = 0);
      this.userProfile.favoriteMovies
        ? (this.favMoviesRemainingChars =
            this.userProfile.favoriteMovies.length)
        : (this.favMoviesRemainingChars = 0);
      this.userProfile.favoriteReads
        ? (this.favrReadsRemainingChars = this.userProfile.favoriteReads.length)
        : (this.favrReadsRemainingChars = 0);
      this.userProfile.favoriteSports
        ? (this.favSportsRemainingChars =
            this.userProfile.favoriteSports.length)
        : (this.favSportsRemainingChars = 0);
      this.userProfile.favoriteCuisines
        ? (this.favCuisinesRemainingChars =
            this.userProfile.favoriteCuisines.length)
        : (this.favCuisinesRemainingChars = 0);
      this.userProfile.favoriteDressStyle
        ? (this.favDressRemainingChars =
            this.userProfile.favoriteDressStyle.length)
        : (this.favDressRemainingChars = 0);
      this.userProfile.favoriteOutings
        ? (this.favOutingsRemainingChars =
            this.userProfile.favoriteOutings.length)
        : (this.favOutingsRemainingChars = 0);
      this.userProfile.tag
        ? (this.tagRemainingChars = this.userProfile.tag.length)
        : (this.tagRemainingChars = 0);
      this.userProfile.phone
        ? (this.phoneRemainingChars = this.userProfile.phone.length)
        : (this.phoneRemainingChars = 0);
      this.userProfile.contactMail
        ? (this.contactMailRemainingChars = this.userProfile.contactMail.length)
        : (this.contactMailRemainingChars = 0);
      this.userProfile.website
        ? (this.websiteRemainingChars = this.userProfile.website.length)
        : (this.websiteRemainingChars = 0);
      this.userProfile.messenger
        ? (this.messangerRemainingChars = this.userProfile.messenger.length)
        : (this.messangerRemainingChars = 0);
      this.userProfile.other
        ? (this.otherRemainingChars = this.userProfile.other.length)
        : (this.otherRemainingChars = 0);
      this.userProfile.hometown
        ? (this.hometownRemainingChars = this.userProfile.hometown.length)
        : (this.hometownRemainingChars = 0);
    }
  }
  saveProfile(showToaster?: boolean) {
    ...
    this.userProfile.profileType = this.AdjustCommasInString(
      this.userProfile.profileType
    );
    !this.userProfile.zipcode
      ? (this.userProfile.zipcode = 0)
      : this.userProfile.zipcode == this.userProfile.zipcode;
  ...
  SaveProfileSeeking() {
    ...
    this.ProfileViewSubscription = this.profileService
      .AddProfileSeeking(this.ProfileSeekingObject)
      .subscribe((result: any) => {
        if (result) {
  ...
  SetProfileSeekingValues() {
    ...
    this.userProfile.id
      ? (this.ProfileSeekingObject.userId = this.userProfile.id)
      : (this.ProfileSeekingObject.userId = 0);
    this.seekingGender != ''
      ? (this.ProfileSeekingObject.gender = this.seekingGender)
      : (this.ProfileSeekingObject.gender = '');
    this.seekingMartialStatus != ''
      ? (this.ProfileSeekingObject.ms = this.seekingMartialStatus)
      : (this.ProfileSeekingObject.ms = '');
    this.seekingChildren != ''
      ? (this.ProfileSeekingObject.children = this.seekingChildren)
      : (this.ProfileSeekingObject.children = '');
    this.seekingSexuality != ''
      ? (this.ProfileSeekingObject.sex = this.seekingSexuality)
      : (this.ProfileSeekingObject.sex = '');
    this.seekingReligion != ''
      ? (this.ProfileSeekingObject.religion = this.seekingReligion)
      : (this.ProfileSeekingObject.religion = '');
    this.seekingEthnicity != ''
      ? (this.ProfileSeekingObject.ethnicity = this.seekingEthnicity)
      : (this.ProfileSeekingObject.ethnicity = '');
    this.seekingPolitical != ''
      ? (this.ProfileSeekingObject.political = this.seekingPolitical)
      : (this.ProfileSeekingObject.political = '');
    this.seekingDisplay != ''
      ? (this.ProfileSeekingObject.disablity = this.seekingDisplay)
      : (this.ProfileSeekingObject.disablity = '');
    this.seekingIncome != ''
      ? (this.ProfileSeekingObject.income = this.seekingIncome)
      : (this.ProfileSeekingObject.income = '');
    this.seekingHeight != ''
      ? (this.ProfileSeekingObject.height = this.seekingHeight)
      : (this.ProfileSeekingObject.height = '');
    this.seekingPhysique != ''
      ? (this.ProfileSeekingObject.physique = this.seekingPhysique)
      : (this.ProfileSeekingObject.physique = '');
    this.seekingEyes != ''
      ? (this.ProfileSeekingObject.eyes = this.seekingEyes)
      : (this.ProfileSeekingObject.eyes = '');
    this.seekingHair != ''
      ? (this.ProfileSeekingObject.hair = this.seekingHair)
      : (this.ProfileSeekingObject.hair = '');
    this.seekingDiet != ''
      ? (this.ProfileSeekingObject.diet = this.seekingDiet)
      : (this.ProfileSeekingObject.diet = '');
    this.seekingDrink != ''
      ? (this.ProfileSeekingObject.drink = this.seekingDrink)
      : (this.ProfileSeekingObject.drink = '');
    this.seekingSmoke != ''
      ? (this.ProfileSeekingObject.smoke = this.seekingSmoke)
      : (this.ProfileSeekingObject.smoke = '');
    this.seekingExercise != ''
      ? (this.ProfileSeekingObject.exercise = this.seekingExercise)
      : (this.ProfileSeekingObject.exercise = '');
    this.seekingLanguages != ''
      ? (this.ProfileSeekingObject.slang = this.seekingLanguages)
      : (this.ProfileSeekingObject.slang = '');
    this.seekingHeightstEdu != ''
      ? (this.ProfileSeekingObject.highestEdu = this.seekingHeightstEdu)
      : (this.ProfileSeekingObject.highestEdu = '');
    this.seekingProfession != ''
      ? (this.ProfileSeekingObject.profession = this.seekingProfession)
      : (this.ProfileSeekingObject.profession = '');
    this.seekingSector != ''
    ...
    this.seekingActivityLevel != ''
      ? (this.ProfileSeekingObject.activityLevel = this.seekingActivityLevel)
      : (this.ProfileSeekingObject.activityLevel = '');
    this.seekingCountry != ''
      ? (this.ProfileSeekingObject.country = this.seekingCountry)
      : (this.ProfileSeekingObject.country = '');
    this.FromAge != '' && this.ToAge != ''
      ? (this.ProfileSeekingObject.ageGroup = this.FromAge + '-' + this.ToAge)
      : (this.ProfileSeekingObject.ageGroup = '');
  }
  async BindProfileCustomFields() {
    if (this.userProfile) {
      this.CustomFieldsSubscription = (
        await this.ProfileCustomFieldsService.getById(this.userProfile.id)
      ).subscribe(
        (result: any) => {
          if (result) {
            this.ProfileCf = result[0];
            if (this.ProfileCf != null && this.ProfileCf != undefined) {
              this.LoadCustomFieldsBoxes(this.ProfileCf);
            }
          }
        },
        (error) => {
  ...
  LoadCustomFieldsBoxes(CustomProfileFields: appProfileCustomFields) {
    ...
    //bind remaining chars
    this.MoreCf.aCustomLabel1
      ? (this.morelbl1RemainingChars = this.MoreCf.aCustomLabel1.length)
      : (this.morelbl1RemainingChars = 0);
    this.MoreCf.aCustomLabel2
      ? (this.morelbl2RemainingChars = this.MoreCf.aCustomLabel2.length)
      : (this.morelbl2RemainingChars = 0);
    this.MoreCf.aCustomLabel3
      ? (this.morelbl3RemainingChars = this.MoreCf.aCustomLabel3.length)
      : (this.morelbl3RemainingChars = 0);
    this.MoreCf.aCustomLabel4
      ? (this.morelbl4RemainingChars = this.MoreCf.aCustomLabel4.length)
      : (this.morelbl4RemainingChars = 0);
    this.MoreCf.aCustomLabel5
      ? (this.morelbl5RemainingChars = this.MoreCf.aCustomLabel5.length)
      : (this.morelbl5RemainingChars = 0);
    this.MoreCf.aCustomLabel6
      ? (this.morelbl6RemainingChars = this.MoreCf.aCustomLabel6.length)
      : (this.morelbl6RemainingChars = 0);
    this.MoreCf.aCustomLabel7
      ? (this.morelbl7RemainingChars = this.MoreCf.aCustomLabel7.length)
      : (this.morelbl7RemainingChars = 0);
    this.MoreCf.aCustomLabel8
      ? (this.morelbl8RemainingChars = this.MoreCf.aCustomLabel8.length)
      : (this.morelbl8RemainingChars = 0);
    this.MoreCf.aCustomText1
      ? (this.moreVlaue1RemainingChars = this.MoreCf.aCustomText1.length)
      : (this.moreVlaue1RemainingChars = 0);
    this.MoreCf.aCustomText2
      ? (this.moreVlaue2RemainingChars = this.MoreCf.aCustomText2.length)
      : (this.moreVlaue2RemainingChars = 0);
    this.MoreCf.aCustomText3
      ? (this.moreVlaue3RemainingChars = this.MoreCf.aCustomText3.length)
      : (this.moreVlaue3RemainingChars = 0);
    this.MoreCf.aCustomText4
      ? (this.moreVlaue4RemainingChars = this.MoreCf.aCustomText4.length)
      : (this.moreVlaue4RemainingChars = 0);
    this.MoreCf.aCustomText5
      ? (this.moreVlaue5RemainingChars = this.MoreCf.aCustomText5.length)
      : (this.moreVlaue5RemainingChars = 0);
    this.MoreCf.aCustomText6
      ? (this.moreVlaue6RemainingChars = this.MoreCf.aCustomText6.length)
      : (this.moreVlaue6RemainingChars = 0);
    this.MoreCf.aCustomText7
      ? (this.moreVlaue7RemainingChars = this.MoreCf.aCustomText7.length)
      : (this.moreVlaue7RemainingChars = 0);
    this.MoreCf.aCustomText8
      ? (this.moreVlaue8RemainingChars = this.MoreCf.aCustomText8.length)
      : (this.moreVlaue8RemainingChars = 0);
  }
  SaveCustomFields() {
    ...
    this.UpdateCustomFieldsSubscription =
      this.ProfileCustomFieldsService.UpdateCusromFields(this.MoreCf).subscribe(
        () => {}
      );
  }
  ReplcaseMultipleSpacesLinesBreak(ismore: boolean) {
    if (ismore) {
      if (this.MoreCf && this.MoreCf !== undefined) {
        this.MoreCf.aCustomLabel1 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomLabel1
        );
        this.MoreCf.aCustomLabel2 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomLabel2
        );
        this.MoreCf.aCustomLabel3 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomLabel3
        );
        this.MoreCf.aCustomLabel4 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomLabel4
        );
        this.MoreCf.aCustomLabel5 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomLabel5
        );
        this.MoreCf.aCustomLabel6 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomLabel6
        );
        this.MoreCf.aCustomLabel7 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomLabel7
        );
        this.MoreCf.aCustomLabel8 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomLabel8
        );
        this.MoreCf.aCustomText1 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomText1
        );
        this.MoreCf.aCustomText2 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomText2
        );
        this.MoreCf.aCustomText3 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomText3
        );
        this.MoreCf.aCustomText4 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomText4
        );
        this.MoreCf.aCustomText5 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomText5
        );
        this.MoreCf.aCustomText6 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomText6
        );
        this.MoreCf.aCustomText7 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomText7
        );
        this.MoreCf.aCustomText8 = this.AdjustMyltipleSpacesLinesBreak(
          this.MoreCf.aCustomText8
        );
      }
    } else {
      this.userProfile.summary = this.AdjustMyltipleSpacesLinesBreak(
        this.userProfile.summary
      );
      this.userProfile.per_Info = this.AdjustMyltipleSpacesLinesBreak(
        this.userProfile.per_Info
      );
      this.userProfile.edu_info = this.AdjustMyltipleSpacesLinesBreak(
        this.userProfile.edu_info
      );
      this.userProfile.profinfo = this.AdjustMyltipleSpacesLinesBreak(
        this.userProfile.profinfo
      );
      this.userProfile.interstinfo = this.AdjustMyltipleSpacesLinesBreak(
        this.userProfile.interstinfo
      );
    }
  }
    ...
    switch (boxName) {
      case 'spoken':
        ...
        if (event.target.checked) {
          ...
          if (
            this.spokenLanguages.length >= 0 &&
            this.spokenLanguages.indexOf(event.target.value) !== -1
          ) {
            this.spokenLanguages = this.spokenLanguages.replace(
              event.target.value,
              ''
            );
          }
          if (
            this.spokenLanguages.length === 1 &&
            this.spokenLanguages.indexOf(',') !== -1
          ) {
      ...
      case 'profiletype':
        ...
        if (event.target.checked) {
          ...
          if (
            this.profileType.length >= 0 &&
            this.profileType.indexOf(event.target.value) !== -1
          ) {
            this.profileType = this.profileType.replace(event.target.value, '');
          }
          if (
            this.profileType.length === 1 &&
            this.profileType.indexOf(',') !== -1
          ) {
            this.profileType = '';
          }
        }
      ...
      case 'seekingGender':
        ...
        if (event.target.checked) {
          ...
          if (this.seekingGender.length >= 0) {
            this.seekingGender.indexOf(event.target.value) != -1
              ? (this.seekingGender = this.seekingGender.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingGender = this.seekingGender);
          ...
          if (
            this.seekingGender.length === 1 &&
            this.seekingGender.indexOf(',') != -1
          ) {
            this.seekingGender = '';
          }
        }
      ...
      case 'seekingMaritalStatus':
        if (event.target.checked) {
          ...
          if (this.seekingMartialStatus.length >= 0) {
            this.seekingMartialStatus.indexOf(event.target.value) != -1
              ? (this.seekingMartialStatus = this.seekingMartialStatus.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingMartialStatus = this.seekingMartialStatus);
          ...
          if (
            this.seekingMartialStatus.length === 1 &&
            this.seekingMartialStatus.indexOf(',') != -1
          ) {
            this.seekingMartialStatus = '';
          }
        }
        this.seekingMartialStatus = this.AdjustCommasInString(
          this.seekingMartialStatus
        );
        this.seekingModified = true;
      ...
      case 'seekingChildren':
        if (event.target.checked) {
          ...
          if (this.seekingChildren.length >= 0) {
            this.seekingChildren.indexOf(event.target.value) != -1
              ? (this.seekingChildren = this.seekingChildren.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingChildren = this.seekingChildren);
          ...
          if (
            this.seekingChildren.length === 1 &&
            this.seekingChildren.indexOf(',') != -1
          ) {
            this.seekingChildren = '';
          }
        }
      ...
      case 'seekingSexuality':
        if (event.target.checked) {
          ...
          if (this.seekingSexuality.length >= 0) {
            this.seekingSexuality.indexOf(event.target.value) != -1
              ? (this.seekingSexuality = this.seekingSexuality.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingSexuality = this.seekingSexuality);
          ...
          if (
            this.seekingSexuality.length === 1 &&
            this.seekingSexuality.indexOf(',') != -1
          ) {
            this.seekingSexuality = '';
          }
        }
      ...
      case 'seekingReligion':
        if (event.target.checked) {
          ...
          if (this.seekingReligion.length >= 0) {
            this.seekingReligion.indexOf(event.target.value) != -1
              ? (this.seekingReligion = this.seekingReligion.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingReligion = this.seekingReligion);
          ...
          if (
            this.seekingReligion.length === 1 &&
            this.seekingReligion.indexOf(',') != -1
          ) {
            this.seekingReligion = '';
          }
        }
      ...
      case 'seekingEthnicity':
        if (event.target.checked) {
          ...
          if (this.seekingEthnicity.length >= 0) {
            this.seekingEthnicity.indexOf(event.target.value) != -1
              ? (this.seekingEthnicity = this.seekingEthnicity.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingEthnicity = this.seekingEthnicity);
          ...
          if (
            this.seekingEthnicity.length === 1 &&
            this.seekingEthnicity.indexOf(',') != -1
          ) {
            this.seekingEthnicity = '';
          }
        }
      ...
      case 'seekingPolitical':
        if (event.target.checked) {
          ...
          if (this.seekingPolitical.length >= 0) {
            this.seekingPolitical.indexOf(event.target.value) != -1
              ? (this.seekingPolitical = this.seekingPolitical.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingPolitical = this.seekingPolitical);
          ...
          if (
            this.seekingPolitical.length === 1 &&
            this.seekingPolitical.indexOf(',') != -1
          ) {
            this.seekingPolitical = '';
          }
        }
      ...
      case 'seekingDisplay':
        if (event.target.checked) {
          ...
          if (this.seekingDisplay.length >= 0) {
            this.seekingDisplay.indexOf(event.target.value) != -1
              ? (this.seekingDisplay = this.seekingDisplay.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingDisplay = this.seekingDisplay);
          ...
          if (
            this.seekingDisplay.length === 1 &&
            this.seekingDisplay.indexOf(',') != -1
          ) {
            this.seekingDisplay = '';
          }
        }
      ...
      case 'seekingHeight':
        if (event.target.checked) {
          ...
          if (this.seekingHeight.length >= 0) {
            this.seekingHeight.indexOf(event.target.value) != -1
              ? (this.seekingHeight = this.seekingHeight.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingHeight = this.seekingHeight);
          ...
          if (
            this.seekingHeight.length === 1 &&
            this.seekingHeight.indexOf(',') != -1
          ) {
            this.seekingHeight = '';
          }
        }
      ...
      case 'seekingPhysique':
        if (event.target.checked) {
          ...
          if (this.seekingPhysique.length >= 0) {
            this.seekingPhysique.indexOf(event.target.value) != -1
              ? (this.seekingPhysique = this.seekingPhysique.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingPhysique = this.seekingPhysique);
          ...
          if (
            this.seekingPhysique.length === 1 &&
            this.seekingPhysique.indexOf(',') != -1
          ) {
            this.seekingPhysique = '';
          }
        }
      ...
      case 'seekingEyes':
        if (event.target.checked) {
          ...
          if (this.seekingEyes.length >= 0) {
            this.seekingEyes.indexOf(event.target.value) != -1
              ? (this.seekingEyes = this.seekingEyes.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingEyes = this.seekingEyes);
          ...
          if (
            this.seekingEyes.length === 1 &&
            this.seekingEyes.indexOf(',') != -1
          ) {
            this.seekingEyes = '';
          }
        }
      ...
      case 'seekingHair':
        if (event.target.checked) {
          ...
          if (this.seekingHair.length >= 0) {
            this.seekingHair.indexOf(event.target.value) != -1
              ? (this.seekingHair = this.seekingHair.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingHair = this.seekingHair);
          ...
          if (
            this.seekingHair.length === 1 &&
            this.seekingHair.indexOf(',') != -1
          ) {
            this.seekingHair = '';
          }
        }
      ...
      case 'seekingDiet':
        if (event.target.checked) {
          ...
          if (this.seekingDiet.length >= 0) {
            this.seekingDiet.indexOf(event.target.value) != -1
              ? (this.seekingDiet = this.seekingDiet.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingDiet = this.seekingDiet);
          ...
          if (
            this.seekingDiet.length === 1 &&
            this.seekingDiet.indexOf(',') != -1
          ) {
            this.seekingDiet = '';
          }
        }
      ...
      case 'seekingDrink':
        if (event.target.checked) {
          ...
          if (this.seekingDrink.length >= 0) {
            this.seekingDrink.indexOf(event.target.value) != -1
              ? (this.seekingDrink = this.seekingDrink.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingDrink = this.seekingDrink);
          ...
          if (
            this.seekingDrink.length === 1 &&
            this.seekingDrink.indexOf(',') != -1
          ) {
            this.seekingDrink = '';
          }
        }
      ...
      case 'seekingSmoke':
        if (event.target.checked) {
          ...
          if (this.seekingSmoke.length >= 0) {
            this.seekingSmoke.indexOf(event.target.value) != -1
              ? (this.seekingSmoke = this.seekingSmoke.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingSmoke = this.seekingSmoke);
          ...
          if (
            this.seekingSmoke.length === 1 &&
            this.seekingSmoke.indexOf(',') != -1
          ) {
            this.seekingSmoke = '';
          }
        }
      ...
      case 'seekingExercise':
        if (event.target.checked) {
          ...
          if (this.seekingExercise.length >= 0) {
            this.seekingExercise.indexOf(event.target.value) != -1
              ? (this.seekingExercise = this.seekingExercise.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingExercise = this.seekingExercise);
          ...
          if (
            this.seekingExercise.length === 1 &&
            this.seekingExercise.indexOf(',') != -1
          ) {
            this.seekingExercise = '';
          }
        }
      ...
      case 'seekingLangs':
        if (event.target.checked) {
          ...
          if (this.seekingLanguages.length >= 0) {
            this.seekingLanguages.indexOf(event.target.value) != -1
              ? (this.seekingLanguages = this.seekingLanguages.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingLanguages = this.seekingLanguages);
          ...
          if (
            this.seekingLanguages.length === 1 &&
            this.seekingLanguages.indexOf(',') != -1
          ) {
            this.seekingLanguages = '';
          }
        }
      ...
      case 'seekingCountry':
        if (event.target.checked) {
          ...
          if (this.seekingCountry.length >= 0) {
            this.seekingCountry.indexOf(event.target.value) != -1
              ? (this.seekingCountry = this.seekingCountry.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingCountry = this.seekingCountry);
          ...
          if (
            this.seekingCountry.length === 1 &&
            this.seekingCountry.indexOf(',') != -1
          ) {
            this.seekingCountry = '';
          }
        }
      ...
      case 'seekingHighestedu':
        if (event.target.checked) {
          ...
          if (this.seekingHeightstEdu.length >= 0) {
            this.seekingHeightstEdu.indexOf(event.target.value) != -1
              ? (this.seekingHeightstEdu = this.seekingHeightstEdu.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingHeightstEdu = this.seekingHeightstEdu);
          ...
          if (
            this.seekingHeightstEdu.length === 1 &&
            this.seekingHeightstEdu.indexOf(',') != -1
          ) {
            this.seekingHeightstEdu = '';
          }
        }
      ...
      case 'seekingProfession':
        if (event.target.checked) {
          ...
          if (this.seekingProfession.length >= 0) {
            this.seekingProfession.indexOf(event.target.value) != -1
              ? (this.seekingProfession = this.seekingProfession.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingProfession = this.seekingProfession);
          ...
          if (
            this.seekingProfession.length === 1 &&
            this.seekingProfession.indexOf(',') != -1
          ) {
            this.seekingProfession = '';
          }
        }
      ...
      case 'seekingSector':
        if (event.target.checked) {
          ...
          if (this.seekingSector.length >= 0) {
            this.seekingSector.indexOf(event.target.value) != -1
              ? (this.seekingSector = this.seekingSector.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingSector = this.seekingSector);
          ...
          if (
            this.seekingSector.length === 1 &&
            this.seekingSector.indexOf(',') != -1
          ) {
            this.seekingSector = '';
          }
        }
      ...
      case 'seekingIndustry':
        if (event.target.checked) {
          ...
          if (this.seekingIndustry.length >= 0) {
            this.seekingIndustry.indexOf(event.target.value) != -1
              ? (this.seekingIndustry = this.seekingIndustry.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingIndustry = this.seekingIndustry);
          ...
          if (
            this.seekingIndustry.length === 1 &&
            this.seekingIndustry.indexOf(',') != -1
          ) {
            this.seekingIndustry = '';
          }
        }
      ...
      case 'seekingProfstatus':
        if (event.target.checked) {
          ...
          if (this.seekingprofStatus.length >= 0) {
            this.seekingprofStatus.indexOf(event.target.value) != -1
              ? (this.seekingprofStatus = this.seekingprofStatus.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingprofStatus = this.seekingprofStatus);
          ...
          if (
            this.seekingprofStatus.length === 1 &&
            this.seekingprofStatus.indexOf(',') != -1
          ) {
            this.seekingprofStatus = '';
          }
        }
      ...
      case 'seekingIncome':
        if (event.target.checked) {
          ...
          if (this.seekingIncome.length >= 0) {
            this.seekingIncome.indexOf(event.target.value) != -1
              ? (this.seekingIncome = this.seekingIncome.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingIncome = this.seekingIncome);
          ...
          if (
            this.seekingIncome.length === 1 &&
            this.seekingIncome.indexOf(',') != -1
          ) {
            this.seekingIncome = '';
          }
        }
      ...
      case 'seekingActivity':
        if (event.target.checked) {
          ...
          if (this.seekingActivityLevel.length >= 0) {
            this.seekingActivityLevel.indexOf(event.target.value) != -1
              ? (this.seekingActivityLevel = this.seekingActivityLevel.replace(
                  event.target.value,
                  ''
                ))
              : (this.seekingActivityLevel = this.seekingActivityLevel);
          ...
          if (
            this.seekingActivityLevel.length === 1 &&
            this.seekingActivityLevel.indexOf(',') != -1
          ) {
            this.seekingActivityLevel = '';
          }
        }
  ...
  LoadProfileSeekings() {
    this.ProfileViewSubscription = this.profileService
      .GetProfileSeeking(this.userProfile.id)
      .subscribe((result: any) => {
        if (result) {
          this.profileSeeking = result[0];
          console.log('profileSeeking gender>> ', this.profileSeeking.gender);
          if (
            this.profileSeeking !== null &&
            this.profileSeeking !== undefined
          ) {
            console.log(
              'this.profileSeeking gender>> ' + this.profileSeeking.gender
            );
  ...
  IsSelectedValues(fieldName: string, value: string): boolean {
    if (this.profileSeeking === null || this.profileSeeking === undefined) {
      ...
      switch (fieldName) {
        case 'age':
          this.profileSeeking.ageGroup.indexOf(value) !== -1
            ? (this.isSelected = true)
            : (this.isSelected = false);
          break;
        case 'gender':
          this.profileSeeking.gender.indexOf(value) !== -1
            ? (this.isSelected = true)
            : (this.isSelected = false);
          break;
      }
  ...
  BindProfileSeekingValues() {
    if (this.profileSeeking != null && this.profileSeeking != undefined) {
      this.profileSeeking.ageGroup != ''
        ? (this.FromAge = this.profileSeeking.ageGroup.substring(
            0,
            this.profileSeeking.ageGroup.indexOf('-')
          ))
        : (this.FromAge = '');
      this.profileSeeking.ageGroup != ''
        ? (this.ToAge = this.profileSeeking.ageGroup.substring(
            this.profileSeeking.ageGroup.indexOf('-') + 1,
            this.profileSeeking.ageGroup.length
          ))
        : (this.ToAge = '');
      console.log('FromAge>> ' + this.FromAge);
      console.log('ToAge>> ' + this.ToAge);
      this.profileSeeking.gender != ''
        ? (this.seekingGender = this.profileSeeking.gender)
        : this.seekingGender == '';
      this.profileSeeking.children != ''
        ? (this.seekingChildren = this.profileSeeking.children)
        : (this.seekingChildren = '');
      this.profileSeeking.sex != ''
        ? (this.seekingSexuality = this.profileSeeking.sex)
        : (this.seekingSexuality = '');
      this.profileSeeking.religion != ''
        ? (this.seekingReligion = this.profileSeeking.religion)
        : (this.seekingReligion = '');
      this.profileSeeking.ethnicity != ''
        ? (this.seekingEthnicity = this.profileSeeking.ethnicity)
        : (this.seekingEthnicity = '');
      this.profileSeeking.political != ''
        ? (this.seekingPolitical = this.profileSeeking.political)
        : (this.seekingPolitical = '');
      this.profileSeeking.disablity != ''
        ? (this.seekingDisplay = this.profileSeeking.disablity)
        : (this.seekingDisplay = '');
      this.profileSeeking.height != ''
        ? (this.seekingHeight = this.profileSeeking.height)
        : (this.seekingHeight = '');
      this.profileSeeking.physique != ''
        ? (this.seekingPhysique = this.profileSeeking.physique)
        : (this.seekingPhysique = '');
      this.profileSeeking.eyes != ''
        ? (this.seekingEyes = this.profileSeeking.eyes)
        : (this.seekingEyes = '');
      this.profileSeeking.hair != ''
        ? (this.seekingHair = this.profileSeeking.hair)
        : (this.seekingHair = '');
      this.profileSeeking.diet != ''
        ? (this.seekingDiet = this.profileSeeking.diet)
        : (this.seekingDiet = '');
      this.profileSeeking.drink != ''
        ? (this.seekingDrink = this.profileSeeking.drink)
        : (this.seekingDrink = '');
      this.profileSeeking.smoke != ''
        ? (this.seekingSmoke = this.profileSeeking.smoke)
        : (this.seekingSmoke = '');
      this.profileSeeking.exercise != ''
        ? (this.seekingExercise = this.profileSeeking.exercise)
        : (this.seekingExercise = '');
      this.profileSeeking.country != ''
        ? (this.seekingCountry = this.profileSeeking.country)
        : (this.seekingCountry = '');
      this.profileSeeking.slang != ''
        ? (this.seekingLanguages = this.profileSeeking.slang)
        : (this.seekingLanguages = '');
      this.profileSeeking.highestEdu != ''
        ? (this.seekingHeightstEdu = this.profileSeeking.highestEdu)
        : (this.seekingHeightstEdu = '');
      this.profileSeeking.profession != ''
        ? (this.seekingProfession = this.profileSeeking.profession)
        : (this.seekingProfession = '');
      this.profileSeeking.sector != ''
        ? (this.seekingSector = this.profileSeeking.sector)
        : (this.seekingSector = '');
      ...
      this.profileSeeking.status != ''
        ...
        : (this.seekingprofStatus = '');
      this.profileSeeking.activityLevel != ''
        ? (this.seekingActivityLevel = this.profileSeeking.activityLevel)
        : (this.seekingActivityLevel = '');
    }
  }
        ...
        .subscribe((result: any) => {
          if (result && result !== undefined) {
            ...
            if (mesg.length > 0) {
              ...
              if (
                mesg[0].deleteIds.length === 0 ||
                mesg[0].deleteIds.indexOf(
  ...
  CheckLimits() {
    this.DailyLimit = this.messagesService.GetLimits(
      'daily',
      this.limits,
      this.upgrade
    );
    this.LifeTimeLimit = this.messagesService.GetLimits(
      'lifetime',
      this.limits,
      this.upgrade
    );
    ...
    this.MessageSubscription = this.messagesService
      ...
      .subscribe((result: any) => {
        if (result) {
          ...
          console.log(
            'check if new thread result length >>' + this.MessageThread.length
          );
          if (this.MessageThread.length === 0) {
            //new thread
            //check limits
            this.CheckLimits();
            this.CheckCreditsSubscription = this.messagesService
              .CheckMessageCredit(this.messagesService.getProfileId())
              .subscribe((result: any) => {
                if (result) {
                  ...
                  if (
                    ...
                    this.NewMessageSubscription = this.messagesService
                      ...
                      .subscribe((result: any) => {
                        ...
                        console.log(
                          'the new inserted message object when new message >> ' +
                            this.InsertedMessage
                        );
                        console.log(
                          'new message ID>> ' + this.InsertedMessage.id
                        );
                        console.log(
                          'the filled new  message thread object>> ' +
                            this.FillNewMessageThread(this.InsertedMessage)
                        );
                        //Add message thread
                        ...
                        this.LikeUnLikeFavUnFav =
                          this.IsLikedService.LikeUnLike(
  ...
  ReplyMessage(threadId) {
    ...
    let message = {
      id: this.ThreadId,
      SenderId: +this.messagesService.getProfileId(),
    };
    this.MessageSubscription = this.messagesService
      .UpdateDeleteMessageStatus(message)
      .subscribe((result: any) => {
        if (result) {
  ...
  SetProfileSeekingGroupValues() {
    ...
    if (this.profileSeeking) {
      ...
      if (this.profileSeeking.physique) {
        if (this.SeekingAppearance.length > 0) {
          //has value
          this.SeekingAppearance +=
            ', ' + this.profileSeeking.physique + ' Physique';
        } else {
      ...
      if (this.profileSeeking.drink) {
        if (this.SeekingLifeStyle.length > 0) {
          //has value
      ...
      if (this.profileSeeking.exercise) {
        if (this.SeekingLifeStyle.length > 0) {
          //has value
          this.SeekingLifeStyle +=
            ', ' + this.profileSeeking.exercise + ' Exercise';
        } else {
      ...
      case 'save':
        switch (imgType) {
          case 'poster':
            ...
            this.updatedProfileImages.User_Id =
            ...
            this.userProfile.canvasImageUrl.length > 1
              ? (this.updatedProfileImages.CanvasImageUrl =
                  this.userProfile.canvasImageUrl.replace('~', ''))
              : (this.updatedProfileImages.CanvasImageUrl =
                  this.userProfile.canvasImageUrl);
            this.updatedProfileImages.imageBase64 = this.resizedImage.replace(
              'data:image/png;base64,',
              ''
            );
            //save image in its folder on the server.
            this.UpdateProfileSubscription = this.profileService
              .UpdateProfilePosterImage(this.updatedProfileImages)
              .subscribe(
                (result: any) => {
                  if (result) {
          ...
          case 'profile':
            ...
            this.userProfile.passportImageUrl.length > 1
              ? (this.updatedProfileImages.PassportImageUrl =
                  this.userProfile.passportImageUrl.replace('~', ''))
              : (this.updatedProfileImages.PassportImageUrl =
                  this.userProfile.passportImageUrl);
            this.updatedProfileImages.imageExtention =
            ...
            //save image in its folder on the server.
            this.UpdateProfileSubscription = this.profileService
              .UpdateProfilePassportImage(this.updatedProfileImages)
              .subscribe(
                (result: any) => {
                  if (result) {
    ...
    switch (action) {
      case 'save':
        ...
        if (this.resizedImage !== '' && this.resizedImage !== undefined) {
          ...
          this.profilePhotoObject.profileId =
            +this.profileService.getProfileId();
          this.profilePhotoObject.imageName = this.imageName;
          this.profilePhotoObject.imageDescription = ' ';
          this.profilePhotoObject.imageAlt =
            this.profileService.getSname() +
            '_' +
            this.profilePhotoObject.imageName;
          this.profilePhotoObject.imageExtention =
            '.' + this.profilePhotoFileExtension;
          this.profilePhotoObject.imageUrl = '';
          this.profilePhotoObject.imageBase64 = this.resizedImage.replace(
            'data:image/png;base64,',
            ''
          );
          this.UpdateProfileSubscription = this.profileService
            .AddProfilePhoto(this.profilePhotoObject)
            .subscribe(
              (result: any) => {
                if (result) {
  ...
  ProfileImageCropped(event: ImageCroppedEvent) {
    ...
    if (event.width < 150 && event.height < 150) {
      ...
      this.isLoadingImageError = true;
      this.loadingimageError =
        "You can't upload images less than 150px width and 150px height.";
      return;
    } else {
  ...
  GetProfilePhotosList() {
    ...
    this.UpdateProfileSubscription = this.profileService
      .GetProfilePhotosList(this.userProfile.id)
      .subscribe((result: any) => {
        if (result) {
          this.profilePhotosList = result;
          for (var i = 0; i < this.profilePhotosList.length; i++) {
            this.PhotosList.push({
              image: this.profileService.bindImageUrl(
                '',
                this.profilePhotosList[i].imageUrl
              ),
              thumbImage: this.profileService.bindImageUrl(
                '',
                this.profilePhotosList[i].imageUrl
              ),
  ...
  ProfilePhotoImageCropped(event: ImageCroppedEvent) {
    ...
    if (event.width < 250 && event.height < 250) {
      ...
      this.isLoadingImageError = true;
      this.loadingimageError =
        "You can't upload images less than 250px width and 250px height.";
      return;
    } else if (event.width > 2400 && event.height > 1200) {
      this.ProfilePhotoChangedEvent = null;
      this.isLoadingImageError = true;
      this.loadingimageError =
        "You can't upload images more than 2400px width and 1200px height.";
      return;
    } else {
  ...
  async CheckLikesFavs(favOrLike: string) {
    ...
    switch (favOrLike) {
      case 'like':
        this.IsLikesSubscription = (
          await this.IsLikedService.create(this.SetProfileLikedFavedObject())
        ).subscribe(
          (data: any) => {
            if (data) {
      ...
      case 'fav':
        this.IsFavsSubscription = (
          await this.IsFavedService.create(this.SetProfileLikedFavedObject())
        ).subscribe(
          (data: any) => {
            if (data) {
              this.ProfileLikedFavedReturnedObject = data[0];
    ...
    switch (favOrLike) {
      case 'like':
        this.LikesSubscription = (
          await this.ProfilesLikedMeService.GetProfileLikesCount(
            this.userProfile.id
          )
        ).subscribe(
          (result: any) => {
      ...
      case 'fav':
        this.FavsSubscription = (
          await this.ProfilesFavMeService.GetProfileFavoritesCount(
            this.userProfile.id
          )
        ).subscribe(
          (result: any) => {
    ...
    switch (favOrLike) {
      case 'like':
        this.CheckCreditsSubscription = (
          await this.profileService.GetProfileLikesCredit(
            this.profileService.getProfileId()
          )
        ).subscribe((result: any) => {
          if (result) {
      ...
      case 'fav':
        this.CheckCreditsSubscription = (
          await this.profileService.GetProfileFavouritsCredit(
            this.profileService.getProfileId()
          )
        ).subscribe((result: any) => {
          if (result) {
