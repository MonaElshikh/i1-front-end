...
export class SupportService extends DataService {
  ...
  async GetTicketsDetials(id: number) {
    return await this._http.get(
      environment.BASE_URL + '/GetTicketDetails/' + id
    );
  }
  ...
  async AddTicketReply(ticket: any) {
    return await this._http.post(
      environment.BASE_URL + '/AddTicketReply',
      ticket
    );
  }
  async GetAdminTickets(adminId: any) {
    return await this._http.get(
      environment.BASE_URL + '/AdminTickets/' + adminId
    );
  }
  async AddAdminTicketReply(ticket: any) {
    return await this._http.post(
      environment.BASE_URL + '/AdminTickets',
      ticket
    );
  }
}
